...
%Due to the presence of $\ez$ in first row of \eqnref{rectifier}
%it is always the case that $H_{13} = 0$. Hence the $x$--cordinate of
%the projection of 3D points $\vect{X}$ is independent of their
%$z$--coordinate. This corresponds to the condition that vertical lines
%appear vertical in images, as originally desired.
...
the horizon line. Equation \eqnref{manhattanhom} defines a homology
...
Next we show how to compute $\HomParam$ from $\zfloor,\zceil$. Let
\begin{eqnarray}
  \efloor = \zfloor ~ \ez \\
  \eceil = \zceil ~ \ez \\
...
second edge in \figref{scene-params}. Ordinarily we will always have
...
Not all models $\Scene$ are physically realisable, but those that are
...
\eqnref{si} and \eqnref{r_i} and find the position of the wall seam at
column $j$,
\begin{equation}
...
which we may apply equations \eqnref{pi},
\eqnref{qi}, \eqnref{ri}, and
\eqnref{si} to recover a polygonal reconstruction. 
...
horizon. This process is formalized in \algref{computing-from-scene}.
...
\algref{seam-depth-orient} \textit{only requires access to the pair
